...
class VesselDataset(torch.utils.data.Dataset):
    ...
    def find_neighboring_files(self, file_path: str, limit: int = 1) -> List:
        ...
        neighboring_files.sort(
            key=lambda x: (
                int(x[len(directory) + len(os.path.sep) + len(prefix) : -len(suffix)])
                if suffix
...
class CutMixDataset(Dataset):
    def __init__(self, dataset, num_mix=1, beta=1.0, prob=1.0):
...
class DataModule(LightningDataModule):
    ...
    def setup(self, stage: Optional[str] = None) -> None:
        ...
        if self.hparams.use_cutmix:
            self.train_dataset = CutMixDataset(dataset=train_dataset)
        else:
            self.train_dataset = train_dataset

        self.valid_dataset = VesselDataset(
            valid_img_paths,
            valid_msk_paths,
        ...
        self.test_dataset = VesselDataset(
            valid_img_paths,
            [],
            transforms=self.transforms["valid"],
            norm_scheme=self.hparams.norm_scheme,
